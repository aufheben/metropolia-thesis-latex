...
different platforms, and utilizes optimizations like \gls{simd} instructions,
...
transposed convolution, $\alpha = 1$, $\beta = 1$, $op(X) = X$, so \gls{gemm} reduces to
...
\gls{gemm} and places them in the destination image. The code is given in Listing \ref{code:col2im}:
...
Here $data\_im$ is the target image while $data\_col$ is the result of \gls{gemm}. The rest is
self-explanatory.  For transposed convolution, $data\_im$ is typically smaller than $data\_col$.
This leads to a key optimization idea: it is possible to merge \gls{gemm} and $col2im$ together and
compute transposed convolution by doing \gls{gemm} sparsely. In doing so, $data\_col$ can be abandoned
and the results will be directly placed in $data\_im$, that is, there is no need for an extra large
