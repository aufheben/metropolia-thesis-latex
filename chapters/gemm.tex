...
transposed convolution on \gls{fpga} with Verilog. It can be seen from this code that the output channels can be computed in groups or even by each channel individually. This is a convenient fact when the weights of a layer cannot be fit into the \gls{bram} on the \gls{fpga} chip all at once.
